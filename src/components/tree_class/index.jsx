import { Tree, Input,message,Modal,Icon } from 'antd';
import React,{ PureComponent } from 'react';

import _ from 'utils/xym_lodash';
import RightMenu,{MenuOption} from '../right_menu';
...
class TreeClass extends PureComponent {
    static defaultProps = {
        name: '图片',
        title: <p className="title">类目管理</p>
    }
    state = {
    ...
    onSelect = (selectedKeys, e) => {
        ...
        this.setState({
            selectedKey
        },() => this.props.getList(this.state.selectedKey))
    };
    ...
    onAdd = (name,type) => {
        ...
        this.props.addClass({...params}).then(res => {
    ...
    onEdit = (type='delete', name='') => {
        ...
        if(type === 'rename') {
            ...
            this.props.putClass({key,name, parentId}).then(res => {
                if(isLeaf) {
            ...
            this.props.deleteClass({key,parentId}).then(res => {
                if(isLeaf) {
                    afterTree = tree.map(item => {
                        if(item._id == parentId) {
    ...
    render() {
        ...
        return (
            <div className={style}>
                ...
                >   <TreeNode title={`全部${name}`} key="all" >
                    {
